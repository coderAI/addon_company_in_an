...
//            'click #ks_add_item_selection>li': '_onKsAdditemTypeClick',
        ...
        init: function (parent,state, params) {
            ...
            this.ks_date_filter_selection_order = ['l_day','t_week','t_month','t_quarter','t_year','n_day','n_week',
                                                    ...
                                                    'ls_quarter','ls_year', 'l_week', 'l_month', 'l_quarter', 'l_year','ls_past_until_now', 'ls_pastwithout_now',
                ...
                var item_data = self.ks_dashboard_data.ks_item_data[items[i]];
                 ...
                 if (item_data['ks_dashboard_item_type'] == 'ks_list_view') {
                    ...
                    if (!self.ks_dashboard_data.ks_item_data[item_data.id]['isDrill']){
                       $(self.$el.find(".grid-stack-item[data-gs-id="+item_data.id+"]")).find('.ks_dashboard_item_drill_up').addClass('d-none');
                    }
...
//        Have  to save dashboard here
        ...
        ksImportItem: function(e) {
            ...
            fileReader.onload = function() {
                ...
                framework.blockUI();
//                ChannelModel.call('channel_invite', [this.channel_id], {partner_ids: _.pluck(data, 'id')})
                new Model('ks_dashboard_ninja.board').call('ks_import_item',[self.ks_dashboard_id],{
        ...
        _ksRenderActiveMode: function () {
           var self = this
            ...
            if (self.grid && Object.keys(self.ks_dashboard_data.ks_item_data).length > 0) {
        ...
        ks_fetch_data: function () {
            ...
            return self.ksCurrentDashboardModel.call('ks_fetch_dashboard_data',
             [self.ks_dashboard_id],{'context':self.getContext()}).done(function(result){
         ...
         ks_fetch_items_data: function(){
            ...
            self.ks_dashboard_data.ks_dashboard_items_ids.forEach(function(value){
                items_promises.push(self.ksCurrentDashboardModel.call('ks_fetch_item',
                    [[value],self.ks_dashboard_id],
                    {'context':self.getContext()}
                    ).then(function(result){
                        self.ks_dashboard_data.ks_item_data[value] = result[parseInt(value)];
                }));
            });
       ...
       renderKpi : function(item, grid){
        ...
        _ksRenderDashboardTile: function (tile){
            ...
            return self._ksRenderTile(tile);
         },

        _ksRenderTile: function(tile){
            var self = this;
                var style_main_body, style_image_body_l2, style_domain_count_body, style_button_customize_body,
                style_button_delete_body ,item_view;
                ...
                switch (tile.ks_layout) {
                    case 'layout1':
                        item_view = QWeb.render('ks_dashboard_item_layout1', {
                            ...
                            ks_icon_url: tile.ks_icon_url,
                            ks_rgba_default_icon_color: ks_rgba_default_icon_color,
                            ks_container_class : tile.ks_container_class,
                            ks_inner_container_class : tile.ks_inner_container_class,
                            ks_dashboard_list: self.ks_dashboard_data.ks_dashboard_list,
                            data_count: tile.data_count,
                            count: tile.count
                        });
                        break;
    ...
    _onKsApplyDateFilter: function (e) {
            ...
            if (start_date === "Invalid date") {
                ...
                self.$el.find(".ks_date_apply_clear_print button").addClass('ks_hide');
...
//            console.log("XLS");
    ...
    ksItemExportJson: function(e) {
            var itemId = $(e.target).parents('.ks_dashboard_item_button_container')[0].dataset.itemId;
            ...
            if (evt.currentTarget.classList.value !== 'ks_list_canvas_click') {
                ...
                if (self.ks_dashboard_data.ks_item_data[item_id].max_sequnce) {
                    ...
                    if ($(evt.target).parent().data().last_seq !== sequence) {
                        new Model('ks_dashboard_ninja.item').call('ks_fetch_drill_down_data',[item_id, domain, sequence])
                            .then(function(result) {
                            if (result.ks_list_view_data) {
                                ...
                                var item_data = self.ks_dashboard_data.ks_item_data[item_id]
                                self._renderChart($(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]), item_data);
                                $($(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]")).find(".card-body")).removeClass("ks_overflow");
                            }
                        });
                    }
                }
    ...
    ksOnDrillUp: function(e) {
            ...
            if(item_data) {
                if ('domains' in item_data) {
                    ...
                    if (sequnce >= 0) {
                        new Model('ks_dashboard_ninja.item').call('ks_fetch_drill_down_data',
                            [item_id, domain, sequnce]).then(function(result) {
                                self.ks_dashboard_data.ks_item_data[item_id]['ks_chart_data'] = result.ks_chart_data;
                                self.ks_dashboard_data.ks_item_data[item_id]['sequnce'] = result.sequence;
                        ...
                        $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]")).find('.ks_dashboard_item_drill_up').addClass('d-none');
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".ks_dashboard_item_chart_info").removeClass('d-none')
                        $(self.$el.find(".grid-stack-item[data-gs-id=" + item_id + "]").children()[0]).find(".ks_dashboard_color_option").removeClass('d-none')
                    ...
                    if(!domain) $(self.$el.find(".grid-stack-item[data-gs-id="+item_id+"]")).find('.ks_dashboard_item_drill_up').addClass('d-none');
        ...
        ksFetchChartItem: function(id) {
           ...
           new Model('ks_dashboard_ninja.board').call('ks_fetch_item',[[item_data.id], self.ks_dashboard_id],{'context':self.getContext()}).
           then(function(new_item_data) {
                ...
                if (item_data.ks_list_view_data) {
                    ...
                    $($(self.$el.find(".grid-stack-item[data-gs-id=" + id + "]")).find(".card-body")).removeClass("ks_overflow");
                }
            }.bind(this));
        },
        ...
        ksLoadMoreRecords: function(e) {
            ...
            if (itemId in self.ksUpdateDashboard) {
                ...
                var offset = {'ks_intial_count': ks_intial_count,
                        'offset': ks_offset}
                new Model('ks_dashboard_ninja.board').call('ks_get_list_view_data_offset',
                    [[parseInt(itemId)], [parseInt(self.ks_dashboard_id)]],{'context':self.getContext(),
                    'offset': offset,
                }).then(function(result) {
