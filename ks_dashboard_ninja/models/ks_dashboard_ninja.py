...
class KsDashboardNinjaBoard(models.Model):
        ...
        for rec in self:
            if 'ks_dashboard_menu_name' in vals:
                ...
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board', False) and self.env.ref(
                        'ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
            ...
            if 'ks_dashboard_group_access' in vals:
                if self.env.ref('ks_dashboard_ninja.ks_my_default_dashboard_board', False) and self.env.ref(
                        'ks_dashboard_ninja.ks_my_default_dashboard_board').id == rec.id:
                    self.env.ref('ks_dashboard_ninja.board_menu_root').sudo().groups_id = vals[
    ...
    def ks_is_dashboard_manager(self, user_id):
        if self.env.ref('ks_dashboard_ninja.ks_dashboard_ninja_group_manager') in self.env['res.users'].browse(
                user_id).groups_id:
            return True
        return False
    ...
    @api.model
    def ks_get_list_view_data_offset(self, ks_dashboard_item_id, dashboard_id, offset):
