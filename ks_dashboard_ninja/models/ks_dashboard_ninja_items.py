...
class KsDashboardNinjaItems(models.Model):
    ...
    ks_model_id = fields.Many2one('ir.model', string='Model', required=True,
                                  domain="[('access_ids','!=',False),('transient','=',False),"
                                         ...
                                         "('model','!=','ks_dashboard_ninja.item_goal'),"
                                         "('model','!=','ks_dashboard_ninja.board_template')]")
    ks_is_model_valid = fields.Boolean()
    check_sudo = fields.Boolean(string="Using Sudo", default=False)
    specail_group_by = fields.Boolean(string="Specail group by", default=False)
    limit_colum = fields.Integer(string="Limit Column")
    ...
    ks_chart_relation_groupby = fields.Many2one('ir.model.fields',
                                                ...
                                                string="Group By")
    ks_chart_relation_specail_group_by = fields.Many2one('ir.model.fields',string="Specail Groupby")
    ks_chart_relation_specail_group_bys = fields.Many2one('ir.model.fields',string="Specail Groupby")
    ...
    #-------------------------------data merger chart
    check_merger_data = fields.Boolean(string="Merger Data")
    ks_merger_model_id_2 = fields.Many2one('ir.model', string='Model',
    ...
    ks_merger_record_field_mapping_2 = fields.Many2one('ir.model.fields',
                                                       domain="[('model_id','=',ks_merger_model_id_2),('name','!=','id'),('store','=',True),'|',('ttype','=','many2one'),('ttype','=','selection')]",
                                                       string="Record Field Mapping Colunm", strore=True)
    ks_merger_model_name_merger_2 = fields.Char(related='ks_merger_model_id_2.model')
    ks_merger_record_count_type_2 = fields.Selection([('sum', 'Sum'),
                                                      ], string="Record Type", default="sum")
    ks_merger_record_field_2 = fields.Many2one('ir.model.fields',
                                               domain="[('model_id','=',ks_merger_model_id_2),('name','!=','id'),('store','=',True),'|','|',('ttype','=','integer'),('ttype','=','float'),('ttype','=','monetary')]",
                                               string="Record Field", strore=True)
    ks_merger_record_count_2 = fields.Float(string="KPI Record Count", compute='ks_get_record_count_2', store=False)

    ks_merger_date_filter_field_2 = fields.Many2one('ir.model.fields',
                                                    domain="[('model_id','=',ks_merger_model_id_2),'|',('ttype','=','date'),('ttype','=','datetime')]",
                                                    string="Date Filter Field")
    ks_merger_item_start_date_2 = fields.Datetime(string="Start Date")
    ks_merger_item_end_date_2 = fields.Datetime(string="End Date")
    ks_merger_domain_2 = fields.Char(string="Domain")
    ...
    ks_target_view = fields.Selection([('Number', 'Number'), ('Progress Bar', 'Progress Bar')], string="View",
                                      default="Number")
    ...
    employee_type = fields.Selection([('all', 'All'),
                                      ('cs', 'Customer Service'),
                                      ('sale', 'Sale'),
                                      ('ts', 'Transaction'),
                                      ('rs', 'Reseller')], default='all', string= 'Employee type')
    ...
    def get_chart_groupby_type(self):
        for rec in self:
            if rec.ks_chart_relation_groupby.ttype == 'datetime' or rec.ks_chart_relation_groupby.ttype == 'date':
                ...
                rec.ks_chart_date_groupby = False
            rec.ks_chart_relation_specail_group_bys = False
        return {'domain': {'ks_chart_relation_specail_group_bys': [('model', '=', self.ks_chart_relation_groupby.relation or '')]}}
        ...
        for rec in self:
            if rec.ks_chart_date_groupby in ('minute', 'hour') or rec.ks_chart_date_sub_groupby in ('minute', 'hour'):
                ...
                return {'warning': {
                    ...
                    'message': _(
                        'Cannot create target lines for Groupby field having aggregation time in  minute and hour')
                }}
    ...
    @api.model
    def ks_view_content_change(self):
        return True
    ...
    @api.depends('ks_chart_measure_field', 'ks_chart_relation_groupby', 'ks_chart_date_groupby', 'ks_domain',
                 ...
                 'ks_chart_relation_specail_group_bys',
                 ...
                 'ks_item_end_date', 'ks_compare_period', 'ks_year_period', 'ks_chart_item_color',
                 'ks_bar_chart_stacked','check_sudo','specail_group_by',
            ...
            if rec.ks_dashboard_item_type and rec.ks_dashboard_item_type != 'ks_tile' and rec.ks_dashboard_item_type != 'ks_list_view' and rec.ks_model_id and rec.ks_chart_data_count_type:
                ...
                if ((rec.ks_chart_data_count_type != "count" and ks_chart_measure_field) or (
                    ...
                    if (
                        ...
                        if rec.ks_chart_relation_specail_group_bys:
                            _logger.info(rec.ks_chart_relation_specail_group_bys)
                        else:
                            if rec.specail_group_by:
                                _logger.info(self.read())
                        _logger.info(rec.specail_group_by)
                        ks_chart_data = rec.ks_fetch_chart_data(rec.ks_model_name, ks_chart_domain,
                                                                ...
                                                                rec.check_merger_data, rec.ks_merger_model_id_2,
                                                                rec.ks_merger_record_field_mapping_2,
                                                                rec.ks_merger_record_field_2,
                                                                rec.ks_merger_date_filter_field_2,
                                                                rec.ks_merger_domain_2,
                                                                rec.ks_chart_relation_specail_group_bys,rec.specail_group_by
                                                                )
    ...
    def ks_fetch_chart_data(self, ks_model_name, ks_chart_domain, ks_chart_measure_field, ks_chart_measure_field_2,
                            ...
                            check_merger_data,ks_merger_model_id_2,ks_merger_record_field_mapping_2,
                            ks_merger_record_field_2,ks_merger_date_filter_field_2,ks_merger_domain_2,
                            ks_chart_relation_specail_group_bys,specail_group_by):
        ...
        try:
            if self.check_sudo:
                ks_chart_records = self.sudo().env[ks_model_name].read_group(ks_chart_domain,
                                                                             set(
                                                                                 ks_chart_measure_field +
                                                                                 ks_chart_measure_field_2
                                                                                 + [ks_chart_groupby_relation_field]),
                ...
                ks_chart_records = self.env[ks_model_name].read_group(ks_chart_domain,
                                                                      ...
                                                                      orderby=orderby,
                                                                      limit=limit)
            if self.limit_colum and self.limit_colum > 0:
                if len(ks_chart_records)> self.limit_colum:
                    ks_chart_records_tmp = []
                    for i in range(0,self.limit_colum):
                        ks_chart_records_tmp.append(ks_chart_records[i])
                    ks_chart_records = ks_chart_records_tmp
        except Exception as e:
            ks_chart_records = []
            pass

        if specail_group_by and ks_chart_relation_specail_group_bys:
            if ks_chart_relation_specail_group_bys.ttype=='many2one':
                ks_chart_records_tmp = []
                memory_add_list = {}
                added_list = []
                running_move = 0
                obj_need_to_convert = self.sudo().env[ks_chart_relation_specail_group_bys.model]
                try:
                    for res in ks_chart_records:
                        if res.get(ks_chart_groupby_field) and res.get(ks_chart_groupby_field)[0]:
                            # lấy dữ liệu chuẩn bị convert
                            specail_field = ks_chart_relation_specail_group_bys.name
                            obj_data = obj_need_to_convert.browse(res.get(ks_chart_groupby_field)[0])
                            value_f = obj_data.read([specail_field])
                            if value_f != []:
                                parent_id  = value_f[0].get(specail_field)[0]
                                if parent_id not in added_list:
                                    # xử lý lần đầu gặp product con của product cha
                                    parent_data = obj_need_to_convert.browse(parent_id)
                                    added_list.append(parent_id)
                                    #ghi lại vùng nhớ của dữ liệu product cha trong mảng
                                    memory_add_list.update({parent_id:running_move})
                                    res_tmp = res.copy()
                                    # xử lý label tên của thằng cha đổi lại domain của thằng cha
                                    res_tmp.update({
                                        ks_chart_groupby_field:(parent_id,parent_data.name or parent_data.id)
                                    })
                                    ks_chart_records_tmp.append(res_tmp)
                                    running_move += 1
                                # xử lý lần gặp product con khác của product cha đã xử lý
                                else:
                                    #lấy product cha tương ứng
                                    item_edit = ks_chart_records_tmp[memory_add_list.get(parent_id)]
                                    #xử lý và update total
                                    for i_total in ks_chart_measure_field:
                                        total = item_edit.get(i_total)
                                        this_total = res.get(i_total)
                                        item_edit.update({i_total:total+this_total})
                        else:
                            ks_chart_records_tmp.append(res)
                            running_move +=1
                    ks_chart_records = ks_chart_records_tmp
                except:
                    ks_chart_records=[]

        ks_merger_data_field_checking=[]
        if check_merger_data:

            ks_merger_data_field_checking.append(ks_merger_record_field_2.name)
            ks_chart_measure_field.append(ks_merger_record_field_2.name)
            ks_chart_measure_field_ids.append(ks_merger_record_field_2.id)
            merger_model_obj = self.sudo().env[ks_merger_model_id_2.name]
            for i_rec in ks_chart_records:
                ...
                i_rec.update({ks_merger_record_field_2.name:total_add})
        ...
        for res in ks_chart_records:
            ...
            if res[ks_chart_groupby_field] and all(measure_field in res for measure_field in ks_chart_measure_field):
                ...
                if ks_chart_measure_field:
                    ...
                    for field_rec in ks_chart_measure_field:
                        ...
                        data = res[field_rec] * ks_groupby_equal_measures \
                            ...
                            res[field_rec] * ks_groupby_equal_measures / \
                            res[ks_chart_groupby_relation_field + "_count"]
                        if field_rec in ks_merger_data_field_checking:
                            try:
                                ks_chart_data['datasets'][counter]['data'].append(data)
                            except:
                                ks_chart_data['datasets'].append({'data': [data], 'label': field_rec})
                        else:
                            ks_chart_data['datasets'][counter]['data'].append(data)
                        counter += 1
                        index += 1
    ...
    def ks_fetch_drill_down_data(self, item_id, domain, sequence):
        ...
        if action_line.ks_chart_type == 'ks_list_view':
            ...
            ks_chart_data = record.ks_fetch_chart_data(record.ks_model_name, domain, ks_chart_measure_field,
                                                       ...
                                                       ks_chart_data,
                                                       record.check_merger_data, record.ks_merger_model_id_2, record.ks_merger_record_field_mapping_2,
                                                       record.ks_merger_record_field_2,
                                                       record.ks_merger_date_filter_field_2, record.ks_merger_domain_2,
                                                       record.ks_chart_relation_specail_group_bys,record.specail_group_by
                                                       )
    ...
    @api.depends('ks_record_count_type', 'ks_model_id', 'ks_domain', 'ks_record_field', 'ks_date_filter_field',
                 ...
                 'ks_dashboard_item_type', 'ks_record_data_limit')
