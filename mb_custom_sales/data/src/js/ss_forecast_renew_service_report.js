odoo.define('mb_custom_sales.ss_forecast_renew_service_report_tag', function (require) {
'use strict';
...
var ReportWidget = require('mb_custom_sales.ReportWidget');
...
var ss_forecast_renew_service_report_tag = Widget.extend(ControlPanelMixin, {
    ...
    get_html: function() {
        var self = this;
        var defs = [];
        return new Model('ss.forecast.renew.service.report.context').call('return_context', [self.report_model, self.given_context, self.report_id]).then(function (result) {
    ...
    render_buttons: function() {
        var self = this;
        this.$buttons = $(QWeb.render("templateReports.buttons", {}));
    ...
    render_searchview_buttons: function() {
        ...
        this.$searchview_buttons = $(QWeb.render("templateReports.searchView", {report_type: this.report_type, context: this.report_context}));
        var $dateFilter = this.$searchview_buttons.siblings('.o_template_reports_date-filter');
        var $dateFilterCmp = this.$searchview_buttons.siblings('.o_template_reports_date-filter-cmp');
        var $useCustomDates = $dateFilter.find('.o_template_reports_use-custom');
        var $CustomDates = $dateFilter.find('.o_template_reports_custom-dates');
        $useCustomDates.bind('click', function () {self.toggle_filter($useCustomDates, $CustomDates);});
        var $usePreviousPeriod = $dateFilterCmp.find('.o_template_reports_use-previous-period');
        var $previousPeriod = $dateFilterCmp.find('.o_template_reports_previous-period');
        $usePreviousPeriod.bind('click', function () {self.toggle_filter($usePreviousPeriod, $previousPeriod);});
        var $useSameLastYear = $dateFilterCmp.find('.o_template_reports_use-same-last-year');
        var $SameLastYear = $dateFilterCmp.find('.o_template_reports_same-last-year');
        $useSameLastYear.bind('click', function () {self.toggle_filter($useSameLastYear, $SameLastYear);});
        var $useCustomCmp = $dateFilterCmp.find('.o_template_reports_use-custom-cmp');
        var $CustomCmp = $dateFilterCmp.find('.o_template_reports_custom-cmp');
        $useCustomCmp.bind('click', function () {self.toggle_filter($useCustomCmp, $CustomCmp);});
        this.$searchview_buttons.find('.o_template_reports_one-filter').bind('click', function (event) {
            self.onChangeDateFilter(event); // First trigger the onchange
            var error = false;
            $('.o_template_reports_datetimepicker input').each(function () { // Parse all the values of the date pickers
                if (error) {return;}
                if ($(this).val() === ""){
        ...
        this.$searchview_buttons.find('.o_template_reports_one-filter-cmp').bind('click', function (event) { // Same for the comparison filter
            self.onChangeCmpDateFilter(event);
            $('.o_template_reports_datetimepicker input').each(function () {
                $(this).val(formats.parse_value($(this).val(), {type: 'date'}));
            });
        ...
        this.$searchview_buttons.find('.o_template_reports_one-filter-bool').bind('click', function (event) { // Same for the boolean filters
            var report_context = {};
        ...
        // fillter company
        this.$searchview_buttons.find('.o_template_reports_one-company').bind('click', function (event) {
            var report_context = {};
            var value = $(event.target).parents('li').data('value');
            console.log(self.report_context.company_ids);
...
//        search category_ids
        this.$searchview_buttons.find(".o_template_reports_analytic_account_auto_complete").select2();
        var selection = [];
        var i;
        for (i = 0; i < this.report_context.category_ids.length; i++) {
            var analytic_category = this.report_context.category_ids[i];
            selection.push({id:analytic_category[0], text:analytic_category[1]});
        }
        this.$searchview_buttons.find('.o_template_reports_analytic_account_auto_complete').data().select2.updateSelection(selection);

        this.$searchview_buttons.find('.o_template_reports_add_analytic_account_tag').bind('click', function (event) {
            var report_context = {};
            var value = self.$searchview_buttons.find(".o_template_reports_analytic_account_auto_complete").select2("val");
            report_context.category_ids = value;
            self.restart(report_context);
        });

//        search not_category_ids
        this.$searchview_buttons.find(".o_template_reports_not_analytic_account_auto_complete").select2();
        var selection = [];
        var i;
        for (i = 0; i < this.report_context.not_category_ids.length; i++) {
            var analytic_not_category = this.report_context.not_category_ids[i];
            selection.push({id:analytic_not_category[0], text:analytic_not_category[1]});
        }
        this.$searchview_buttons.find('.o_template_reports_not_analytic_account_auto_complete').data().select2.updateSelection(selection);
        this.$searchview_buttons.find('.o_template_reports_not_add_analytic_account_tag').bind('click', function (event) {
            var report_context = {};
            var value = self.$searchview_buttons.find(".o_template_reports_not_analytic_account_auto_complete").select2("val");
            report_context.not_category_ids = value;
            self.restart(report_context);
        });


//        search customer_type
        this.$searchview_buttons.find(".o_template_reports_customer_type_auto_complete").select2();
        var selection = [];
        var i;
        for (i = 0; i < this.report_context.customer_type.length; i++) {
            var analytic_customer_type = this.report_context.customer_type[i];
            selection.push({id:analytic_customer_type[0], text:analytic_customer_type[1]});
        }
        this.$searchview_buttons.find('.o_template_reports_customer_type_auto_complete').data().select2.updateSelection(selection);
        this.$searchview_buttons.find('.o_template_reports_customer_type_tag').bind('click', function (event) {
            var report_context = {};
            var value = self.$searchview_buttons.find(".o_template_reports_customer_type_auto_complete").select2("val");
            report_context.customer_type = value;
            self.restart(report_context);
        });
        ...
        var $datetimepickers = this.$searchview_buttons.find('.o_template_reports_datetimepicker');
    ...
    onChangeCmpDateFilter: function(event, fromDateFilter) {
        ...
        if (filter_cmp === 'previous_period' || filter_cmp === 'same_last_year') {
            ...
            if (!no_date_range) {
                this.$searchview_buttons.find("input[name='date_from_cmp']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(moment(dtFrom));
            }
            this.$searchview_buttons.find("input[name='date_to_cmp']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(moment(dtTo));
        }
    },
    onChangeDateFilter: function(event) {
        ...
        switch($(event.target).parents('li').data('value')) {
            case 'today':
                dt = new Date();
                self.$searchview_buttons.find("input[name='date_to']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(moment(dt));
                break;
            case 'last_month':
                dt = new Date();
                ...
                if (!no_date_range) {
                    ...
                    self.$searchview_buttons.find("input[name='date_from']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(moment(dt));
                }
                break;
            case 'last_quarter':
                dt = new Date();
                ...
                break;
            case 'last_year':
                if (today.getMonth() + 1 < 12 || (today.getMonth() + 1 === 12 && today.getDate() <= 31)) {
                    dt = new Date(today.getFullYear() - 1, 11, 31, 12, 0, 0, 0);
                }
                else {
                    dt = new Date(today.getFullYear(), 11, 31, 12, 0, 0, 0);
                }
                $("input[name='date_to']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(moment(dt));
                if (!no_date_range) {
            ...
            case 'this_quarter':
                dt = new moment();
                self.$searchview_buttons.find("input[name='date_to']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(dt.endOf('quarter'));
                if (!no_date_range) {
                    self.$searchview_buttons.find("input[name='date_from']").parents('.o_template_reports_datetimepicker').data("DateTimePicker").setValue(dt.startOf('quarter'));
                }
                break;
            case 'this_year':
                if (today.getMonth() + 1 < 12 || (today.getMonth() + 1 === 12 && today.getDate() <= 31)) {
...
core.action_registry.add("ss_forecast_renew_service_report_tag", ss_forecast_renew_service_report_tag);
return ss_forecast_renew_service_report_tag;
});
